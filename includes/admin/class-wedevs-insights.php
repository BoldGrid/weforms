...
if ( !class_exists( 'WeDevs_Insights' ) ) {
    ...
    class WeDevs_Insights {
        ...
        public function __construct( $slug, $name, $file, $notice = '' ) {
            ...
            add_action( 'plugin_action_links_' . $this->basename, [ $this, 'plugin_action_links' ] );
            add_action( 'admin_footer', [ $this, 'deactivate_scripts' ] );

            // clean events and options on deactivation
            register_deactivation_hook( $file, [ $this, 'deactivate_plugin' ] );

            // uninstall reason
            add_action( 'wp_ajax_' . $this->slug . '_submit-uninstall-reason', [ $this, 'uninstall_reason_submission' ] );

            // cron events
            add_action( 'cron_schedules', [ $this, 'add_weekly_schedule' ] );
            add_action( $this->slug . '_tracker_send_event', [ $this, 'send_tracking_data' ] );
        ...
        private function send_request( $params, $route ) {
            $resp = wp_remote_post( self::$api_url . $route, [
                'method'      => 'POST',
                'timeout'     => 45,
                'redirection' => 5,
                'httpversion' => '1.0',
                'blocking'    => false,
                'headers'     => [ 'user-agent' => 'WeDevsTracker/' . md5( esc_url( home_url() ) ) . ';' ],
                'body'        => $params,
                'cookies'     => [],
            ]
          );
        ...
        public function admin_notice() {
            ...
            if ( !$this->is_local_server() ) {
                ...
                if ( empty( $this->notice ) ) {
                    $notice = sprintf( __( 'Want to help make <strong>%s</strong> even more awesome? Allow weDevs to collect non-sensitive diagnostic data and usage information.', 'weforms' ), $this->name );
                } else {
                    $notice = $this->notice;
                }

                $notice .= ' (<a class="insights-data-we-collect" href="#">' . __( 'what we collect', 'weforms' ) . '</a>)';
        ...
        public function add_weekly_schedule( $schedules ) {
            ...
            'display'  => __( 'Once Weekly', 'weforms' ),
        ];
