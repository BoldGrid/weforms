...
abstract class Weforms_REST_Controller extends WP_REST_Controller {
    ...
    public function get_items_permissions_check( $request ) {
        if ( ! current_user_can( weforms_form_access_capability() ) ) {
            return new WP_Error( 'rest_weforms_forbidden_context', __( 'Sorry, you are not allowed','weforms' ), array( 'status' => rest_authorization_required_code() ) );
        }
   ...
   */
    ...
    */
    ...
    public function is_form_exists( $param, $request, $key ) {
        ...
        if( empty( $result ) ) {
            return new WP_Error( 'rest_weforms_form', __( 'Form Not Exist','weforms' ), array( 'status' => 404 ) );
        } else {
            return true;
        }
    }
